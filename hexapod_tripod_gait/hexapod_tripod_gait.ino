...
void loop() {
  ...
  int gait_resolution = 20;
  for (int i=0; i<num_legs; ++i) {
    if (group1(i)) {
      gait_leg_offset[i] = 0;
    } else {
      gait_leg_offset[i] = (gait_resolution / 2);
    }
  }
  ...
  while(true) {
    for (int i=0; i<gait_resolution; ++i) {
      ...
      for (int leg=0; leg<num_legs; ++leg) {
        ...
        if (leg_gait_pos < gait_resolution / 2) {
          legs[leg].setHorizontalAngle(80);
          legs[leg].setLateralAngle(map(leg_gait_pos, 0, gait_resolution / 2 - 1, 45, -45));
        } else {
          legs[leg].setHorizontalAngle(map(leg_gait_pos, gait_resolution / 2, gait_resolution - 1, 30, 80));
          legs[leg].setLateralAngle(map(leg_gait_pos, gait_resolution / 2, gait_resolution - 1, -45, 45));
        }
      }
